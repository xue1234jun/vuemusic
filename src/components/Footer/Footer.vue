  ...
  <transition name="fade">
    <div v-show="isShowMiniMusic" :style="{backgroundColor: skinColor}" class="footer">
      <div class="mini-music">
        <div class="music-img">
          <img @click="showPlay" ref="img" v-bind:src="audio.musicImgSrc || (musicData[0]&&musicData[0].musicImgSrc) || defaultImg" alt="">
        </div>
        <div class="music-name">
          ...
          <span class='lrc'>{{lrcfirst||'lrc'}}</span>
        </div>
...
export default {
      ...
      setInterval(() => {
        ...
        if(this.lrcdata.length !== 0)
        {
            for (var i = 0; i < this.lrcdata.length; i++) {
              if ((i + 1) === (this.lrcdata.length - 1)) {
                  this.lrcfirst=this.lrcdata[i][1];
                  break;
              }
              if (this.now > this.lrcdata[i][0] - 0.5 && this.now < this.lrcdata[i + 1][0] - 0.5) {
                  this.lrcfirst=this.lrcdata[i][1];
                  break;
              }
          }
  ...
  computed: {
    lrcdata(){
      return this.$store.state.lrcresult;
    },
    isPlaying() {
